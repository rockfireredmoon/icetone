...
public class TextField extends Element
	...
	public TextField(ElementManager screen, String UID) {
	...
	public TextField(ElementManager screen, Vector2f dimensions, Vector4f resizeBorders, String defaultImg) {
	 ...
	 */
	public TextField(ElementManager screen, String UID, Vector2f position, Vector2f dimensions, Vector4f resizeBorders,
			String defaultImg) {
	...
	public void onKeyPress(KeyInputEvent evt) {
		if (evt.getKeyCode() == KeyInput.KEY_F1 || evt.getKeyCode() == KeyInput.KEY_F2 || evt.getKeyCode() == KeyInput.KEY_F3
			...
			if (caretIndex > -1) {
				...
				if ((Screen.isMac() && !alt) || (Screen.isWindows() && !ctrl) || (Screen.isUnix() && !ctrl)
	...
	public void onKeyRelease(KeyInputEvent evt) {
		...
		evt.setConsumed();
		controlKeyReleaseHook(evt, getText());
	}

	/**
	 * An overridable hook for the onKeyRelease event of the TextField
	 * 
	 * @param evt
	 *            KeyInputEvent
	 * @param text
	 ...
	 * 
	 * @param text
	 *            String The text to set for the TextField
	 */
	@Deprecated
	...
	private void setTextRangeEnd(int tail) {
		if (!visibleText.equals("") && rangeHead != -1) {
			...
			if (rangeHead >= this.head)
				rangeX = getAbsoluteX() + rangeX + getTextPaddingVec().x;
			else
				rangeX = getTextPaddingVec().x;
			...
			float rangeW = getTextPaddingVec().y;
			...
			if (rangeTail <= this.tail) {
				...
				rangeW = getAbsoluteX() + width + getTextPaddingVec().y;
			}
