...
public class StyleManager {
	public static enum CursorType {
		POINTER, HAND, MOVE, TEXT, WAIT, RESIZE_CNW, RESIZE_CNE, RESIZE_NS, RESIZE_EW, CUSTOM_0, CUSTOM_1, CUSTOM_2, CUSTOM_3, CUSTOM_4, CUSTOM_5, CUSTOM_6, CUSTOM_7, CUSTOM_8, CUSTOM_9
	}
			...
			try {
				...
				for (int s = 0; s < nLst.getLength(); s++) {
					...
					if (fstNode.getNodeType() == Node.ELEMENT_NODE) {
						...
						JmeCursor jmeCursor = (JmeCursor) app.getAssetManager().loadAsset(curPath);// new
																									// JmeCursor();
						try {
						...
						cursors.put(CursorType.valueOf(key), (JmeCursor) app.getAssetManager().loadAsset(curPath));
					}
				}
			} catch (Exception ex) {
				...
				for (int s = 0; s < nLst.getLength(); s++) {
					...
					if (fstNode.getNodeType() == Node.ELEMENT_NODE) {
						...
						try {
							...
							System.err.println("Problem parsing attributes: " + key + ". ");
							ex.printStackTrace();
						}
	...
	private void addStyleTag(Style style, Node nNode, Element nElmnt) {
		...
		if (type.equals("Vector2f")) {
			...
			style.putTag(name, Float.parseFloat(XMLHelper.getNodeAttributeValue(nNode, "value")));
		} else if (type.equals("int")) {
			style.putTag(name, Integer.parseInt(XMLHelper.getNodeAttributeValue(nNode, "value")));
		} else if (type.equals("boolean")) {
			style.putTag(name, Boolean.parseBoolean(XMLHelper.getNodeAttributeValue(nNode, "value")));
		} else if (type.equals("String")) {
			style.putTag(name, XMLHelper.getNodeAttributeValue(nNode, "value"));
		} else if (type.equals("Effect")) {
			style.putTag(name,
					new Effect(
							Effect.EffectType.valueOf(
									XMLHelper.getNodeAttributeValue(nElmnt.getElementsByTagName("effect").item(0), "value")),
					Effect.EffectEvent
							.valueOf(XMLHelper.getNodeAttributeValue(nElmnt.getElementsByTagName("event").item(0), "value")),
