...
import icetone.controls.extras.ChatBox;
import icetone.controls.extras.ChatBoxExt;
...
import icetone.controls.windows.DialogBox;
import icetone.controls.windows.LoginBox;
...
import icetone.core.utils.XMLHelper;
...
//		controls.add(SelectBox.class);
			...
			if (method != null) {
				...
				for (Class paramClass : params) {
					if (paramClass == Integer.class || paramClass == Integer.TYPE) {
						...
						finalParams[index] = (Menu) screen
								.getElementById(XMLHelper.getNodeAttributeValue(childNode, "param" + index));
	...
	private Element createElement(Element el, Class c, String id, String pos, String dim, String rsb, String img, String ori,
			String flag, final NodeList childNodes) {
		...
		if (className.equals("ComboBox"))
			styleName = "TextField";
		if (className.equals("TabControl") || className.equals("LoginBox") || className.equals("DialogBox")
				|| className.equals("AlertBox") || className.equals("ChatBox") || className.equals("ChatBoxExt")
		...
		// Orientation orientation = Orientation.valueOf(ori);
		// Boolean bool = Boolean.parseBoolean(flag);
		// return new Spinner<String>(screen, id, position, dimensions,
		// resizeBorders, img, orientation, bool) {
		// @Override
		...
		// if (XMLHelper.getNodeAttributeValue(childNode,
		// "name").equals("onChange")) {
		// try {
		// for (Method method : state.getClass().getDeclaredMethods()) {
		// if
		// (method.getName().equals(XMLHelper.getNodeAttributeValue(childNode,
		// "stateMethodName")))
		...
		// }
			...
			Boolean bool = Boolean.parseBoolean(flag);
			return new Slider<Float>(screen, id, position, dimensions, resizeBorders, img, orientation, bool) {
				@Override
							...
							if (childNode.getNodeName().equals("eventMethod")) {
								if (XMLHelper.getNodeAttributeValue(childNode, "name").equals("onChange")) {
									try {
										for (Method method : state.getClass().getDeclaredMethods()) {
											if (method.getName()
													.equals(XMLHelper.getNodeAttributeValue(childNode, "stateMethodName")))
												method.invoke(state.getClass().cast(state), valueToIndex(value), value);
										}
									} catch (Exception ex) {
										ex.printStackTrace();
									}
...
//		} 
			...
			return new SelectList(screen, id, position, dimensions, resizeBorders, img, true) {
				@Override
				protected void onListItemClicked(int menuItemIndex, Object value, boolean isToggled) {
					for (int n = 0; n < childNodes.getLength(); n++) {
						org.w3c.dom.Node childNode = childNodes.item(n);
						if (childNode.getNodeType() == org.w3c.dom.Node.ELEMENT_NODE) {
							if (childNode.getNodeName().equals("eventMethod")) {
		...
		for (int n = 0; n < childNodes.getLength(); n++) {
			...
			if (childNode.getNodeType() == org.w3c.dom.Node.ELEMENT_NODE) {
				if (childNode.getNodeName().equals("effect")) {
					...
					case SlideIn:
					case SlideOut:
						effect.setEffectDirection(
								Effect.EffectDirection.valueOf(XMLHelper.getNodeAttributeValue(childNode, "direction")));
						break;
					case SlideTo:
