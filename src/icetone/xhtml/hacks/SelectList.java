...
public abstract class SelectList extends ScrollArea implements MouseMovementListener, MouseButtonListener, TabFocusListener, KeyboardListener {
	...
	public int addListItem(String caption, Object value) {
		...
		SelectList.ListItem listItem = new SelectList.ListItem(
			this,
			caption,
			value
		);
	...
	public void pack() {
		...
		for (SelectList.ListItem mi : listItems) {
			...
			//	highlight.setClippingLayer(this);
				...
				LUtil.setPosition(highlight, 0, scrollableArea.getHeight()-((listItems.size()-index)*listItemHeight) + listPadding.x + listPadding.w);
		...
		LUtil.setBounds(scrollableArea, listPadding.y, listPadding.x, ((getWidth() > width) ? getWidth() : width)-(listPadding.y + listPadding.z), currentHeight);
	...
	private void displayHighlights() {
		...
		for (SelectList.ListItem mi : listItems) {
			...
			//	highlight.setClippingLayer(this);
				highlight.addClippingLayer(this);
				highlight.setClipPadding(listPadding);
				highlight.setY(scrollableArea.getHeight()-((listItems.size()-index)*listItemHeight) + listPadding.x + listPadding.w);
				scrollableArea.addChild(highlight);
			}
			currentHeight += listItemHeight;
			index++;
	...
	private Element createHighlight(int index) {
		Element highlight = new Element(
			...
			new Vector2f(0,0),
			new Vector2f(listPadding.y + listPadding.z,listPadding.x + listPadding.w),
