...
public class TGGUserAgent implements UserAgentCallback {
	...
	protected ImageResource createImageResource(String uri, final InputStream is) {
		try {
			...
			if (Screen.isAndroid()) {
				loaderClazz = (Class<? extends AssetLoader>) getClass().getClassLoader()
						.loadClass("com.jme3.texture.plugins.AndroidImageLoader");
			} else {
				loaderClazz = (Class<? extends AssetLoader>) getClass().getClassLoader()
						.loadClass("com.jme3.texture.plugins.AWTLoader");
			}
			Image img = (Image) loaderClazz.newInstance()
					.load(new AssetInfo(screen.getApplication().getAssetManager(), new AssetKey(uri)) {
						@Override
