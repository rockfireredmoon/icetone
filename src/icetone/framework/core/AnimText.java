...
public class AnimText extends AnimElement {
	public static enum TagType {
		Paragraph, NewLine, Underline, StrikeThrough, Italic, Bold
	}
	...
	public final void setText(String text) {
		...
		for (int i = 0; i < text.length(); i++) {
			...
			if (bc != null) {
				if (c != ' ') {
					tr = addTextureRegion(String.valueOf(c.hashCode()), bc.getX(), imgHeight - bc.getY() - bc.getHeight(),
	...
	public void wrapTextNoWrap() {
		int i = 0;
		x = 0;
		y = -(font.getCharSet().getBase() / 2) * size;
		lnWidth = 0;
		lIndex = 0;
		bcSpc = font.getCharSet().getCharacter('i');
		for (char c : characters) {
			...
			if (bc != null) {
				...
				if (c != ' ') {
					...
					if (hasLines && ul)
						line.setWidth(line.getWidth() + (bcSpc.getXAdvance() * size));
				}
			}
			i++;
		}
			...
			if (bc != null) {
				if (c == ' ' && !newLine) {
					...
					if (x + bc.getXAdvance() > width || newLine) {
						...
						x = 0;
						if (newLine)
							y -= font.getCharSet().getBase() * newLineCount;
						else
		...
		for (char c : characters) {
			...
			if (bc != null) {
				...
				if (c == ' ') {
					...
					wordSIndex = lIndex;
					if (hasLines && ul)
						line.setWidth(line.getWidth() + (bcSpc.getXAdvance() + 4 * size));
				} else {
					QuadData quad = letters[lIndex];
					float offset = font.getCharSet().getBase() * size;
					offset -= (bc.getHeight() * size);
...
//		lineCount = (int) ((Math.abs(y) / lineHeight));
	...
	private void placeWord(float width) {
		if (lnWidth + wordWidth <= width) {
			...
			for (int w = wordSIndex; w <= wordEIndex; w++) {
				QuadData quad = letters[w];
				quad.setPositionY(quad.getPositionY() - (font.getCharSet().getBase() * size));
				if (hasLines)
...
//		float fontOffset = 0;
		...
		switch (textVAlign) {
		case Top:
			setPositionY((int)(height - offset.y-( lnHeight / 2f)) + offset.y);
			if (hasLines)
		...
		case Center:
			...
			case NoWrap:
			case Clip:
				setPositionY((int)(fontOffset + ( ( height - offset.y ) / 2)  - ( ( lnHeight - offset.y ) / 2)));
				if (hasLines)
					lineDisplay.setOriginY(0);
				break;
			default:
				setPositionY(( (int)(((height - offset.y ) / 2f) + ( ( innerHeight - offset.y ) / 2f ) )));
				if (hasLines)
					lineDisplay.setOriginY(0);
				break;
			}
		...
		case Bottom:
			...
			case Clip:
				...
				break;
			default:
				setPositionY((int)(innerHeight + (lnHeight / 2f ) ) - offset.y);
				if (hasLines)
					lineDisplay.setOriginY(0);
				break;
			}
