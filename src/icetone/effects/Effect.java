...
public class Effect implements Cloneable {
	...
	private void initSlides() {
		...
		System.out.println("DIFF: " + diff + " " + element.getAbsoluteY());
	}
	...
	private void updateFadeIn() {
		...
		if (pass >= 1.0) {
			...
			if (element.getTextElement() != null) {
				element.getTextElement().setAlpha(1f * element.getAlphaFactor());
			}
			if (element instanceof TextElement)
				((TextElement)element).setAlpha(element.getAlphaFactor());
			isActive = false;
		} else {
			float val = pass;
			if (val <= 0.0f)
			...
			if (element.getTextElement() != null) {
				element.getTextElement().setAlpha(val * element.getAlphaFactor());
			}
			if (element instanceof TextElement)
				((TextElement)element).setAlpha(val * element.getAlphaFactor());
		}
	}
	...
	private void updateFadeOut() {
		...
		if (pass >= 1.0) {
			if (!destroyOnHide) {
				...
				if (element.getTextElement() != null) {
					element.getTextElement().setAlpha((0.01f) * element.getAlphaFactor());
				}
				if (element instanceof TextElement)
			...
			if (element.getTextElement() != null) {
				element.getTextElement().setAlpha((1.0f-pass) * element.getAlphaFactor());
			}
			if (element instanceof TextElement)
				((TextElement)element).setAlpha((1.0f-pass) * element.getAlphaFactor());
		}
	}
