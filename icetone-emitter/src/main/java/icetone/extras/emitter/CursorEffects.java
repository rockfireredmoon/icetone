...
public class CursorEffects {
	...
	private Vector2f posSwap = new Vector2f();
	
	public CursorEffects(BaseScreen screen) {
		this.screen = screen;
		...
		cursorEmitter.setSprite("icetone/particles/core.png", 3, 3, 8);
	...
	public void updatePosition(Vector2f position) {
		posSwap.setX(position.x).setY(screen.getHeight() - position.y);
		cursorEmitter.setPosition(posSwap );
	}
	...
	public void start() {
		...
		cursorEmitter.getParticles().setLocalTranslation(0, 0, Float.MAX_VALUE);
		isActive = true;
	}
