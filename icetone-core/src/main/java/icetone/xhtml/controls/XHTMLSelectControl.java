...
public class XHTMLSelectControl extends XHTMLControl {

	private List<String> values, labels;

	public XHTMLSelectControl(FormControl control, XHTMLRenderer parent, LayoutContext c, CalculatedStyle style,
	...
	private SelectList<String> makeList(XHTMLRenderer parent, final SelectControl sc) {
		...
		list.setPreferredRows(sc.getSize());
		list.onChanged(evt -> {
			if (!list.isAdjusting()) {
				list.runAdjusting(() -> {
					if (sc.isMultiple()) {
