...
public abstract class XHTMLControl implements XHTMLFormControl {
	...
	public XHTMLControl(FormControl control, XHTMLRenderer parent, LayoutContext c, CalculatedStyle style,
	...
	protected void configureCommonAttributes(CalculatedStyle style, FormControl control) {
		tgElement.setFont(getFont().getFontSpec());
	...
	private static ColorRGBA convertColor(FSColor fsColor) {
		if (fsColor instanceof FSRGBColor) {
			...
			return new ColorRGBA(fsrgbcolor.getRed() / 255f, fsrgbcolor.getGreen() / 255f, fsrgbcolor.getBlue() / 255f,
					1f);
		} else {
