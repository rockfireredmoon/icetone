...
public class AnimElementMesh extends Mesh {
	...
	private void addQuad(QuadData qd) {
		 ...
		 * 
		 * applyTransforms(qd, dim.x+skew.x, dim.y+skew.y); vb.put(index+9,
		 * tempV.x); vb.put(index+10,tempV.y);
		 * vb.put(index+11,qd.getPositionZ());
		 * 
		 * if (buildColor) { index = qd.index*16; int indexX = 0; for (int i =
		 * 0; i < 4; i++) { cb.put(index+indexX, qd.getColorR());
		 * cb.put(index+indexX+1, qd.getColorG()); cb.put(index+indexX+2,
		 * qd.getColorB()); cb.put(index+indexX+3, qd.getColorA()); indexX += 4;
		 * } }
		 * 
		 ...
		 * 
		 * if (buildIndices) { index = qd.index*6; ib.put(index,
		 * (short)(vCount+2)); ib.put(index+1, (short)(vCount)); ib.put(index+2,
		 * (short)(vCount+1)); ib.put(index+3, (short)(vCount+1));
		 * ib.put(index+4, (short)(vCount+3)); ib.put(index+5,
		 * (short)(vCount+2)); }
		 * 
		 * vCount += 4;
		 */
